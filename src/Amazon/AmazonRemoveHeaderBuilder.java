...
public class AmazonRemoveHeaderBuilder implements Value<Map<Value<String>, Value<String>>> {
    private final Value<URL> endpoint;
    private final DateStamp stamp;
    private final Text region;

    public AmazonRemoveHeaderBuilder(Value<URL> endpoint, DateStamp stamp, Text region) {
    ...
    public Map<Value<String>, Value<String>> get() {
        ...
        headers.put(new StringText("Authorization"), new AuthorizationHeader(endpoint, new StringText("DELETE"), headers, stamp, region));
        return headers;
    }
}
